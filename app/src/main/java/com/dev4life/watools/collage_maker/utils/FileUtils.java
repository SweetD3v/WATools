package com.dev4life.watools.collage_maker.utils;
...
public class FileUtils {
    ...
    public static File saveBitmapAsFile(Bitmap bitmap, String dirName) {
        ...
        File file2 = new File(file + "/" + WAToolsApp.Companion.getInstance().getString(R.string.app_name) + "/" + dirName);
        if (!file2.exists()) {
            file2.mkdirs();
        }
        try {
            File file3 = new File(file + "/" + WAToolsApp.Companion.getInstance().getString(R.string.app_name) + "/" + dirName + "/" + new SimpleDateFormat("yyyyMMdd_HHmmss", Locale.ENGLISH).format(new Date()) + ".jpg");
            file3.createNewFile();
