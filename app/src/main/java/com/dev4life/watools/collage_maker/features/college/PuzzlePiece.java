package com.dev4life.watools.collage_maker.features.college;
    ...
    public void fillArea(View view, boolean z) {
        if (!isFilledArea()) {
            ...
            this.animator.addUpdateListener(valueAnimator -> {
                ...
                float f1 = (matrixScale + ((minMatrixScale - matrixScale) * floatValue)) / matrixScale;
                float f21 = f4 * floatValue;
                float f31 = f5 * floatValue;
                PuzzlePiece.this.zoom(f1, f1, pointF);
                PuzzlePiece.this.postTranslate(f21, f31);
                view2.invalidate();
            });
            if (z) {
